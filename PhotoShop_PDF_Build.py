...
images_foulder = os.path.normpath(os.path.join(os.path.dirname(base_image_foulder),"Merged"))

image_files = glob.glob(images_foulder+"/*.jpg")
...
app.preferences.rulerUnits = ps_api.Units.Pixels

# Create the document
#docRef = app.documents.add(1280,960, 72.0, "Contact_Sheet")


#target Photoshop

make_layercomp = """function main(){

		if(!documents.length) return;

	var doc = activeDocument;
	doc.layerComps.add("Name_to_Replace", "",false, false, true);
	}
...
docRef = app.load(image_file_path)

with Session(docRef, auto_close=False) as ps:
	...
	for i in range(ps.active_document.layers.length):
		hide_all_layers(ps.active_document.layers)
		layer = ps.active_document.layers[i]
		layer.visible = True
		app.doJavaScript(make_layercomp.replace("Name_to_Replace",layer.name))
